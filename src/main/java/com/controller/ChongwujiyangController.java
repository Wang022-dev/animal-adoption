...
//            chongwujiyang.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        if(chongwujiyangEntity==null){
            ...
            publishApprovedAnimal(chongwujiyang);
            return R.ok();
        }else {
            return R.error(511,"表中有相同数据");
        }
    ...
    private void publishApprovedAnimal(ChongwujiyangEntity chongwujiyang) {
        if(chongwujiyang == null || chongwujiyang.getId() == null || chongwujiyang.getChongwujiyangYesnoTypes() == null) {
            return;
        }
        // 字典中 2 表示审核通过。救助申报通过后，自动进入动物档案和领养展示。
        if(chongwujiyang.getChongwujiyangYesnoTypes() != 2) {
            return;
        }
        ChongwujiyangEntity saved = chongwujiyangService.selectById(chongwujiyang.getId());
        if(saved == null) {
            saved = chongwujiyang;
        }

        Wrapper<ChongwuEntity> chongwuWrapper = new EntityWrapper<ChongwuEntity>()
            .eq("chongwu_name", saved.getChongwuName())
            .eq("chongwu_photo", saved.getChongwuPhoto());
        if(chongwuService.selectOne(chongwuWrapper) == null) {
            ChongwuEntity chongwu = new ChongwuEntity();
            chongwu.setChongwuName(saved.getChongwuName());
            chongwu.setChongwuPhoto(saved.getChongwuPhoto());
            chongwu.setChongwuTypes(saved.getChongwuTypes());
            chongwu.setChongwuContent(buildAnimalContent(saved));
            chongwu.setCreateTime(new Date());
            chongwuService.insert(chongwu);
        }

        Wrapper<ChongwulingyangEntity> lingyangWrapper = new EntityWrapper<ChongwulingyangEntity>()
            .eq("chongwulingyang_name", saved.getChongwuName())
            .eq("chongwulingyang_photo", saved.getChongwuPhoto());
        if(chongwulingyangService.selectOne(lingyangWrapper) == null) {
            ChongwulingyangEntity lingyang = new ChongwulingyangEntity();
            lingyang.setChongwulingyangName(saved.getChongwuName());
            lingyang.setChongwuTypes(saved.getChongwuTypes());
            lingyang.setChongwulingyangPhoto(saved.getChongwuPhoto());
            lingyang.setJieshuTypes(1);
            lingyang.setChongwulingyangContent(buildAnimalContent(saved) + "<br/>救助审核已通过，当前可提交领养申请。");
            lingyang.setCreateTime(new Date());
            chongwulingyangService.insert(lingyang);
        }
    }

    private String buildAnimalContent(ChongwujiyangEntity saved) {
        StringBuilder content = new StringBuilder();
        content.append("来源：用户流浪动物申报。");
        if(saved.getJiyangdizhi() != null) {
            content.append("<br/>发现地点：").append(saved.getJiyangdizhi());
        }
        if(saved.getLianxirenName() != null || saved.getLianxirenPhone() != null) {
            content.append("<br/>申报联系人：")
                .append(saved.getLianxirenName() == null ? "" : saved.getLianxirenName())
                .append(" ")
                .append(saved.getLianxirenPhone() == null ? "" : saved.getLianxirenPhone());
        }
        if(saved.getChongwujiyangContent() != null) {
            content.append("<br/>申报详情：").append(saved.getChongwujiyangContent());
        }
