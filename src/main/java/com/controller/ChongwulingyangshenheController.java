...
//            chongwulingyangshenhe.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        if(chongwulingyangshenheEntity==null){
            ...
            updateAdoptionStatus(chongwulingyangshenhe);
            return R.ok();
        }else {
            return R.error(511,"表中有相同数据");
        }
    ...
    private void updateAdoptionStatus(ChongwulingyangshenheEntity shenhe) {
        if(shenhe == null || shenhe.getChongwulingyangId() == null || shenhe.getChongwulingyangshenheYesnoTypes() == null) {
            return;
        }
        ChongwulingyangEntity lingyang = chongwulingyangService.selectById(shenhe.getChongwulingyangId());
        if(lingyang == null) {
            return;
        }
        // 字典中 2 表示审核通过，3 表示拒绝。通过后标记为已认领；拒绝后恢复可认领。
        if(shenhe.getChongwulingyangshenheYesnoTypes() == 2) {
            lingyang.setJieshuTypes(2);
            chongwulingyangService.updateById(lingyang);
        } else if(shenhe.getChongwulingyangshenheYesnoTypes() == 3) {
